...
IF EXISTS(
   ...
   BEGIN
    PRINT 'NÃO temos inicialização instantânea de arquivos.'
    PRINT 'Conceda à conta de serviços do SQL Server a política de segurança '' Executar Tarefas de Manutenção de Volume ''.'
   END
ELSE
   BEGIN
    PRINT 'Temos inicialização de arquivo instantânea.'
   END