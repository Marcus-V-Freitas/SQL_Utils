...
-- Percentual  de média de fragmentação (avg_fragmentation_in_percent)
declare @menorOuIgual30 varchar(15) = 'reorganize'
...
select	 db_name(database_id) as 'database name'
		...
		,case 
			when (ips.avg_fragmentation_in_percent <= 30) 
				then 'alter index '+i.name+' on ['+db_name(database_id)+'].['+s.name+'].['+t.name+'] '+@menorOuIgual30+''
			when (ips.avg_fragmentation_in_percent > 30) 
