...
SELECT db_name(rqt.database_id),rqt.session_id,rqt.status,blocking_session_id,wait_type,last_wait_type,wait_resource,wait_time,original_login_name,TEXT
      FROM sys.dm_exec_requests rqt
      INNER JOIN sys.dm_exec_connections cnt
            ON rqt.session_id = cnt.session_id 
